...
def get_facts_by_date(facts_dicts, filter_date=None):
    ...
    """
    result = []
    if filter_date is None or isinstance(filter_date, str) or filter_date == "":
        return result

    fd_split = str.split(filter_date, ' ')
    if len(fd_split[0]) != 4 or len(fd_split[0]) != 2 or len(fd_split[0]) != 2:
        return result

    facts_num = 0
    for fact in facts_dicts:
        if "createdAt" in fact:
            date = (fact["createdAt"][:10]).replace("-", " ")
            if filter_date == date:
                result.append(fact)
                facts_num += 1
                print("#" + str(facts_num) + " this fact is about " + fact["type"] + ":")
